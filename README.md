...
純前端台股分析工具。GitHub Actions 會定時產生熱門 40 檔清單與日線 JSON，前端使用下拉選單挑選單一標的後，用線性迴歸趨勢估算與技術指標產生分析結果、PNG 圖表與 Markdown 報告。
...
分析結果只供研究參考，不是保證獲利建議，也不是個人化投資建議。線性迴歸速度較快，但無法理解新聞、籌碼與突發事件。資料來源若變更格式或限制存取，GitHub Actions 更新可能失敗。
